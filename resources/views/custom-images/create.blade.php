                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-info">
                            <strong>{{ __('custom_images.cost') }}:</strong> {{ $tokenCost }}
                            {{ __('custom_images.token') }}<br>
                            <strong>{{ __('custom_images.current_balance') }}:</strong> {{ $userBalance }}
                            {{ __('custom_images.token') }}
                        </div>
                        ...
                        <form method="POST" action="{{ route('custom-images.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label for="prompt" class="form-label">{{ __('custom_images.prompt_label') }} <span
                                        class="text-danger">*</span></label>
                                <textarea class="form-control @error('prompt') is-invalid @enderror" id="prompt" name="prompt" rows="4"
                                    required placeholder="{{ __('custom_images.prompt_placeholder') }}">{{ old('prompt') }}</textarea>
                            ...
                            <div class="mb-3">
                                <label for="reference_images"
                                    class="form-label">{{ __('custom_images.reference_images_label') }}</label>
                                <div class="custom-file-wrapper">
                                    ...
                                    <label for="reference_images" class="custom-file-label">
                                        ...
                                        <span class="custom-file-text"
                                            id="file-label-text">{{ __('custom_images.no_file_chosen') }}</span>
                                    </label>
                                </div>
                                <small class="form-text text-muted">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    {{ __('custom_images.create_button') }} ({{ $tokenCost }}
                                    {{ __('custom_images.token') }})
                                </button>
                            </div>
                        </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fileInput.addEventListener('change', async function() {
                ...
                fileLabelText.textContent = '⏳ Görseller hazırlanıyor...';
                ...
                for (let i = 0; i < this.files.length; i++) {
                    ...
                    try {
                        ...
                        cardBody.innerHTML = '<small class="text-muted">' + file.name + '</small><br>' +
                            '<small class="text-muted">' + (renamedFile.size / 1024 / 1024).toFixed(2) +
                            ' MB</small>';
                ...
                fileLabelText.textContent = dt.files.length +
                    ' {{ __('custom_images.reference_images_label') }}';

                const container = document.createElement('div');
                container.className = 'alert alert-success';
                container.innerHTML =
                    '<strong>{{ __('custom_images.selected_images', ['count' => '']) }}'.replace(
                        ':count', dt.files.length) + '</strong>';
